...
class BasicNavigator(Node):
    ...
    def __init__(self):
        ...
        self.nav_to_pose_client = ActionClient(self, action_type=NavigateToPose, action_name='navigate_to_pose')
        self.follow_waypoints_client = ActionClient(self, action_type=FollowWaypoints, action_name='follow_waypoints')
        self.follow_path_client = ActionClient(self, action_type=FollowPath, action_name='follow_path')
        self.compute_path_to_pose_client = ActionClient(self, action_type=ComputePathToPose, action_name='compute_path_to_pose')
        self.spin_client = ActionClient(self, action_type=Spin, action_name='spin')
        self.backup_client = ActionClient(self, action_type=BackUp, action_name='backup')

        self.localization_pose_sub = self.create_subscription(msg_type=PoseWithCovarianceStamped,
                                                              topic='amcl_pose',
                                                              callback=self._amclPoseCallback,
                                                              qos_profile=amcl_pose_qos)
        
        self.initial_pose_pub = self.create_publisher(msg_type=PoseWithCovarianceStamped,
                                                      topic='initialpose',
                                                      qos_profile=10)
        
        self.change_maps_srv = self.create_client(srv_type=LoadMap, srv_name='/map_server/load_map')

        self.clear_costmap_global_srv = self.create_client(srv_type=ClearEntireCostmap, srv_name='/global_costmap/clear_entirely_global_costmap')
        self.clear_costmap_local_srv = self.create_client(srv_type=ClearEntireCostmap, srv_name='/global_costmap/clear_entirely_local_costmap')

        self.get_costmap_global_srv = self.create_client(srv_type=GetCostmap, srv_name='/global_costmap/get_costmap')
        self.get_costmap_local_srv = self.create_client(srv_type=GetCostmap, srv_name='/local_costmap/get_costmap')
