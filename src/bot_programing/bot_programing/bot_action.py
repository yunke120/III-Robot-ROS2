...
from bot_programing.bot_navigator import BasicNavigator, TaskResult
...
class Bot_Cmd():
    ...
    def waitMove(self):
        ...
        while not self.navigator.isTaskComplete():
            ...
            if Duration.from_msg(feedback.navigation_time) > Duration(seconds=6000.0):
    ...
    def move_base(self, x , y, theta):
        self.goal_pose.header.frame_id = 'base_link_1'
        self.goal_pose.header.stamp = self.navigator.get_clock().now().to_msg()
        self.goal_pose.pose.position.x = x
        self.goal_pose.pose.position.y = y
