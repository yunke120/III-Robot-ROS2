...
def generate_launch_description():
    ...
    # 或者
    # ros2 launch cpp06_urdf display.launch.py model:=$(ros2 pkg prefix --share cpp06_urdf)/urdf/urdf/demo01_helloworld2.urdf
    model = DeclareLaunchArgument(name="model", default_value=get_package_share_directory("robot_model") + "/urdf/xacro/robot.urdf.xacro")
    ...
    rviz2 = Node(
        package='rviz2', 
        executable='rviz2',
        arguments=["-d", get_package_share_directory("robot_model") + "/rviz/urdf.rviz"]    
    )
    # return LaunchDescription([model, rviz2])
